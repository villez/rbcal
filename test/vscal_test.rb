...
class VsCalTest < Minitest::Test
  def test_single_month
    ...
    assert_output(expected) do
      VsCal.new(Date.new(2011, 03), Date.new(2011, 03)).print_calendar
    end
  end
 ...
 def test_full_year
   ...
   assert_output(expected) do
      VsCal.new(Date.new(2007, 01), Date.new(2007, 12)).print_calendar
    end
  end
  ...
  def test_month_range_within_year
    ...
    assert_output(expected) do
      VsCal.new(Date.new(2011, 02), Date.new(2011, 06)).print_calendar
    end
  end
  ...
  def test_month_range_multiple_years
    ...
    assert_output(expected) do
      VsCal.new(Date.new(1995, 10), Date.new(1996, 02)).print_calendar
    end
  end
end
